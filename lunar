...
def shigan(rigan,shizhi):
    ...
    if key2=="亥":
        shizhishu=12
    shiganshu=riganshu*2+shizhishu-2
    shiganshu=str(shiganshu)
    print("是干书",shiganshu[-1:])
    if shiganshu[-1:]=="1":
        shigan="甲"
    if shiganshu[-1:]=="2":
        shigan="乙"
...
def shiganzhi(ct=None):
    def shigan(rigan,shizhi):
        ...
        return shigan       
    ln = Lunar(ct)
    shigan=shigan(ln.gz_day(),ln.gz_hour())
    shiganzhi=str(shigan)+str(ln.gz_hour())
    return shiganzhi
def jieqi(ct=None):
    ln = Lunar(ct)
    return ln.ln_jie()
...
def yuegan(ct=None):
	...
	if ln.ln_month()==12:
		zhi="丑"
	niangan=ln.gz_year()
	if niangan[0]=="甲" or niangan[0]=="己":
		shouyuegan="丙"
	if niangan[0]=="乙" or niangan[0]=="庚":
		shouyuegan="戊"
	if niangan[0]=="丙" or niangan[0]=="辛":
		shouyuegan="庚"
	if niangan[0]=="丁" or niangan[0]=="壬":
		shouyuegan="壬"
	if niangan[0]=="戊" or niangan[0]=="癸":
		shouyuegan="甲"
	xulie=["甲","乙","丙","丁","戊","己","庚","辛","壬","癸"]
	for x in range(0,len(xulie)):
		if xulie[x]==shouyuegan:
			key=x
	if ln.ln_month()==1:
		key=key
		if key>10:
	...
	return yuegan+zhi
	# print("月干",yuegan+zhi)
# yuegan(datetime.datetime(2019,1,20,20,20,20))
